$( document ).ready(function() {
  ...
  $('#btn-close').on('click' , function(){
     $('.compare-popup-section').slideUp(200);
  });

  $('.cart-popup-section').hide();
  $('.cartMenuDropdown').on('click' , function(){ 
    $('body').toggleClass('mobile-nav-active');
    if($('.cart-popup-section').is(":visible")){
      $('.cart-popup-section').slideUp(200);
    }else{
      $('.cart-popup-section').slideDown(150); 
      
      //$('.mobile-nav-panel .mobile-nav-toggle').click();
    }
  });
  ...
  $('.payment-options input[name="payment-method"]').on('change' , function(){
    console.log(this.value)
    if(this.value == 0) {
      $('#pickup-details').hide();
      $('#delivery-details .pick-delivery-content').hide();
      $('#delivery-details').show();
    } else if(this.value == 1) {
      $('#pickup-details, #pickup-details h4.c2').show();
      $('#delivery-details').hide();
      $('#pickup-details .pick-delivery-content').hide();
      $('#pickup-details h4.c1').hide();
    } else {
      $('#pickup-details').show();
      $('#delivery-details').show();
      $('#pickup-details .pick-delivery-content').show();
      $('#delivery-details .pick-delivery-content').show();
      $('#pickup-details h4.c2').hide();
      $('#pickup-details h4.c1').show();
    }
  });
  ...
  // Confirmation  
  $('#confirmationpopup').on('shown.bs.modal', function (e) {
    // do something...
    console.log('hi'); 
    $('#okConfirmButton').on('click' , function(){
      $('#orderdetailpopup').modal('show');
      $('#confirmationpopup').modal('hide');
    })
  })

  $('#orderdetailpopup').on('shown.bs.modal', function (e) {
    $('body').addClass('modal-open');
  })

  $('#orderdetailpopup').on('hidden.bs.modal', function (e) {
